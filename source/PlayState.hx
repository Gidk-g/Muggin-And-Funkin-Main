...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (SONG.song.toLowerCase() == 'full-glass') {
			filters.push(ShadersHandler.chromaticAberration);
			ShadersHandler.setChrome(1.5 / 1000);
		}

		if (SONG.song.toLowerCase() == 'pumped') {
			filters.push(ShadersHandler.chromaticAberration);
			ShadersHandler.setChrome(1.5 / 1000);
		}

		if (SONG.song.toLowerCase() == 'double-down') {
			filters.push(ShadersHandler.chromaticAberration);
			ShadersHandler.setChrome(1.5 / 1000);
		}
		...
		switch (curStage)
		{
			case 'house': //Cuphead Shit
				var bg:BGSprite = new BGSprite('cup/house/house', -200, -200);
        ...
        if (SONG.song.toLowerCase() == 'full-glass')
        {
			...
			cupshid.frames = Paths.getSparrowAtlas('funny/CUpheqdshid');
			cupshid.animation.addByPrefix('cupshid', 'Cupheadshit_gif instance', 24, true);
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					schoolIntro(doof);
                case 'full-glass' | 'pumped' | 'double-down':
                    startDialogue(dialogueJson, 'music');
